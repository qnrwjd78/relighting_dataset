"""Render Objaverse object datasets with randomized lighting parameters."""
...
DATASET_UTILS = Path(__file__).resolve().parent / "download"
...
try:
    ...
    raise SystemExit("scripts/render_objaverse_random_dataset.py must be run by Blender Python.") from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Override the config receiver texture manifest.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-relighting-masks",
        dest="relighting_masks",
        action="store_false",
        default=True,
        help="Disable relighting auxiliary white occ/noocc renders and derived mask labels.",
    )
...
def configure_object_rig_scale(
    config: dict,
    object_min_size: float,
    object_target_size: float,
    rig_reference_object_size: float,
    canonical_world_scale: float,
) -> None:
    object_config = config.setdefault("object", {})
    object_config["min_size"] = float(object_min_size)
    object_config["target_size"] = float(object_target_size)
    canonical = config.setdefault("canonical", {})
    canonical["reference_object_size"] = float(rig_reference_object_size)
    canonical["world_scale"] = float(canonical_world_scale)
    canonical["scale_multiplier_range"] = [1.0, 1.0]
...
def configure_receiver_textures(
    config: dict,
    root: Path,
    ...
    configured = (
        manifest_override
        or config.get("receiver_texture_manifest")
        or config.get("layout", {}).get("receiver_texture_manifest")
        or DEFAULT_RECEIVER_TEXTURE_MANIFEST
    )
    manifest_path = resolve_path(root, configured)
    loaded = load_receiver_texture_manifest(manifest_path, root) if not disabled and manifest_path.is_file() else []
    usable = []
    for texture in loaded:
        albedo = texture.get("maps", {}).get("albedo")
        albedo_path = albedo.get("path") if isinstance(albedo, dict) else None
        if albedo_path and Path(albedo_path).is_file():
            usable.append(texture)

    try:
        manifest_value = str(manifest_path.relative_to(root.resolve()))
    except ValueError:
    ...
    status = {
        ...
        "missing_albedo_count": len(loaded) - len(usable),
    }
    runtime = config.setdefault("_runtime", {})
    runtime["receiver_textures"] = usable
    runtime["receiver_texture_status"] = status
    return status
...
def import_asset_or_primitive(asset_path: str | None, primitive: str, rng: random.Random, config: dict) -> list[bpy.types.Object]:
    ...
    if asset_path and hsrd100_asset and orientation_mode.lower() in {"keep", "none", "off"}:
        orientation_mode = "hsrd100_upright"
    orientation_meta = normalize_objects(
        mesh_objects,
        float(config["object"].get("min_size", config["object"].get("target_size", 1.2))),
        float(config["object"].get("target_size", 1.2)),
        orientation_mode,
    )
    runtime = config.setdefault("_runtime", {})
...
def choose_object_orientation(objects: list[bpy.types.Object], mode: str) -> tuple[str, Matrix, str]:
    ...
    if mode in {"negative_y_to_z", "minus_y_to_z"}:
        return "negative_y_to_z", Matrix.Rotation(math.radians(-90.0), 4, "X"), mode
    if mode == "hsrd100_upright":
        matrix = Matrix.Rotation(math.radians(180.0), 4, "Z") @ Matrix.Rotation(math.radians(-90.0), 4, "X")
        return "negative_y_to_z_front_yaw180", matrix, mode
...
def normalize_objects(
    ...
    clamped_size = min(max(max_dim, min_size), max_size)
    scale = clamped_size / max_dim
    ...
    return {
        ...
        "original_max_extent": float(max_dim),
        "clamped_max_extent": float(clamped_size),
        "scale": float(scale),
        "size_range": [float(min_size), float(max_size)],
    }
...
def relighting_mask_config(config: dict) -> dict:
    raw = config.get("relighting_masks", {})
    raw = dict(raw) if isinstance(raw, dict) else {}
    defaults = {
        "effect_percentile": 95.0,
        "effect_threshold_fraction": 0.03,
        "effect_geom_threshold_fraction": 0.05,
        "preserve_effect_threshold_fraction": 0.12,
        "preserve_effect_geom_threshold_fraction": 0.15,
        "lit_threshold_fraction": 0.25,
        "absolute_luminance_threshold": 1.0e-4,
        "hf_blur_sigma": 1.0,
        "hf_threshold_fraction": 0.40,
        "shadow_boundary_blur_sigma": 0.75,
        "preserve_dilate_radius": 7,
        ...
        "contact_soft_sigma": 12.0,
        "contact_soft_threshold": 0.15,
...
def relighting_white_shading_config(config: dict) -> dict:
    mask_cfg = relighting_mask_config(config)
    white_config = dict(config.get("_pbr_white_shading", pbr_white_shading_config(config)))
    white_config["enabled"] = True
    white_config["mode"] = str(mask_cfg.get("white_mode", "direct")).lower()
    white_config["direct_only"] = white_config["mode"] != "optical"
    white_config["caustics"] = bool(white_config["mode"] == "optical")
    white_config["bounces"] = {} if white_config["direct_only"] else white_config.get("bounces", {})
    white_config.setdefault("force_white_light", True)
    white_config["preserve_alpha_cutout"] = bool(mask_cfg.get("preserve_alpha_cutout", True))
    white_config["material"] = "white_diffuse_optical" if white_config["mode"] == "optical" else "white_diffuse_direct"
    return white_config
...
def load_render_rgb(path: Path) -> np.ndarray:
    ...
    try:
        ...
        pixels = array("f", [0.0]) * (width * height * channels)
        image.pixels.foreach_get(pixels)
        raw = np.asarray(pixels, dtype=np.float32).reshape((height, width, channels))
        rgb = np.zeros((height, width, 3), dtype=np.float32)
        for channel in range(3):
            rgb[..., channel] = raw[..., min(channel, channels - 1)]
        return np.nan_to_num(rgb, nan=0.0, posinf=0.0, neginf=0.0).clip(min=0.0)
    finally:
        bpy.data.images.remove(image)
...
def positive_percentile(values: np.ndarray, percentile: float, eps: float) -> float:
    finite = values[np.isfinite(values)]
    if finite.size == 0:
        return float(eps)
    q_all = float(np.percentile(finite, percentile))
    positive = finite[finite > eps]
    if positive.size == 0:
        return max(q_all, float(eps))
    q_pos = float(np.percentile(positive, percentile))
    return max(q_all, q_pos, float(eps))


def normalize_by_percentile(values: np.ndarray, percentile: float, eps: float) -> tuple[np.ndarray, float]:
    scale = positive_percentile(values, percentile, eps)
    return np.clip(values / (scale + eps), 0.0, 1.0).astype(np.float32), scale


def laplacian_abs(values: np.ndarray) -> np.ndarray:
...
def gaussian_blur(values: np.ndarray, sigma: float) -> np.ndarray:
    ...
    padded_x = np.pad(values, ((0, 0), (radius, radius)), mode="edge")
    tmp = np.zeros_like(values, dtype=np.float32)
    for index, weight in enumerate(kernel):
        tmp += float(weight) * padded_x[:, index : index + width]

    padded_y = np.pad(tmp, ((radius, radius), (0, 0)), mode="edge")
    out = np.zeros_like(values, dtype=np.float32)
    for index, weight in enumerate(kernel):
        out += float(weight) * padded_y[index : index + height, :]
    return out
...
def dilate_binary(mask: np.ndarray, radius: int) -> np.ndarray:
    ...
    for y_offset in range(size):
        for x_offset in range(size):
            out |= padded[y_offset : y_offset + height, x_offset : x_offset + width]
    return out


def chamfer_distance_to_mask(seed: np.ndarray) -> np.ndarray:
    seed = seed.astype(bool)
    height, width = seed.shape
    inf = float(height + width + 1024)
    dist = np.where(seed, 0.0, inf).astype(np.float32)
    diag = math.sqrt(2.0)
    for y in range(height):
        for x in range(width):
            best = dist[y, x]
            if y > 0:
                best = min(best, float(dist[y - 1, x]) + 1.0)
                if x > 0:
                    best = min(best, float(dist[y - 1, x - 1]) + diag)
                if x + 1 < width:
                    best = min(best, float(dist[y - 1, x + 1]) + diag)
            if x > 0:
                best = min(best, float(dist[y, x - 1]) + 1.0)
            dist[y, x] = best
...
def save_gray_png(path: Path, values: np.ndarray) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    gray = np.nan_to_num(values.astype(np.float32), nan=0.0, posinf=1.0, neginf=0.0)
    gray = np.clip(gray, 0.0, 1.0)
    height, width = gray.shape
    image = bpy.data.images.new(f"TL_label_{path.stem}", width=width, height=height, alpha=True, float_buffer=False)
    try:
        ...
        rgba[..., 2] = gray
        rgba[..., 3] = 1.0
        image.pixels.foreach_set(array("f", rgba.reshape(-1).tolist()))
        image.update()
...
def save_gray_npy(path: Path, values: np.ndarray) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    np.save(path, np.nan_to_num(values.astype(np.float32), nan=0.0, posinf=1.0, neginf=0.0))


def save_soft_label(scene_dir: Path, rel_base: str, values: np.ndarray, labels: dict, key: str) -> None:
    npy_rel = f"{rel_base}.npy"
    png_rel = f"{rel_base}.png"
    save_gray_npy(scene_dir / npy_rel, values)
    save_gray_png(scene_dir / png_rel, values)
    labels[key] = npy_rel
    labels[f"{key}_png"] = png_rel


def save_binary_label(scene_dir: Path, rel_path: str, mask: np.ndarray, labels: dict, key: str) -> None:
    save_gray_png(scene_dir / rel_path, mask.astype(np.float32))
    labels[key] = rel_path
...
def build_relighting_labels(
    scene_dir: Path,
    light_index: int,
    color_output: dict,
    white_occ_output: dict,
    white_noocc_output: dict,
    config: dict,
) -> dict:
    cfg = relighting_mask_config(config)
    eps = float(cfg["eps"])
    color_exr = component_output_exr_path(scene_dir, color_output)
    white_occ_exr = component_output_exr_path(scene_dir, white_occ_output)
    white_noocc_exr = component_output_exr_path(scene_dir, white_noocc_output)
    missing = [
        name
        for name, path in (
            ("spotlight_color_occ", color_exr),
            ("spotlight_white_occ", white_occ_exr),
            ("spotlight_white_noocc", white_noocc_exr),
        )
        if path is None or not path.exists()
    ]
    if missing:
        return {
            "schema": "tokenlight_relighting_labels_v2",
            "valid": False,
            "error": f"missing_exr:{','.join(missing)}",
        }

    color_lum = luminance(load_render_rgb(color_exr))
    white_occ_lum = luminance(load_render_rgb(white_occ_exr))
    white_noocc_lum = luminance(load_render_rgb(white_noocc_exr))

    abs_thr = float(cfg["absolute_luminance_threshold"])
    effect_color_soft, effect_color_scale = normalize_by_percentile(color_lum, float(cfg["effect_percentile"]), eps)
    effect_color_thr = max(abs_thr, float(cfg["effect_threshold_fraction"]) * effect_color_scale)
    effect_color_bin = color_lum > effect_color_thr
    preserve_effect_color_thr = max(abs_thr, float(cfg["preserve_effect_threshold_fraction"]) * effect_color_scale)
    preserve_effect_color_bin = color_lum > preserve_effect_color_thr

    effect_geom_soft, effect_geom_scale = normalize_by_percentile(white_occ_lum, float(cfg["effect_percentile"]), eps)
    effect_geom_thr = max(abs_thr, float(cfg["effect_geom_threshold_fraction"]) * effect_geom_scale)
    effect_geom_bin = white_occ_lum > effect_geom_thr
    preserve_effect_geom_thr = max(abs_thr, float(cfg["preserve_effect_geom_threshold_fraction"]) * effect_geom_scale)
    preserve_effect_geom_bin = white_occ_lum > preserve_effect_geom_thr

    white_noocc_scale = positive_percentile(white_noocc_lum, float(cfg["effect_percentile"]), eps)
    possible_thr = max(abs_thr, float(cfg["possible_threshold_fraction"]) * white_noocc_scale)
    possible = white_noocc_lum > possible_thr

    lit_thr = max(abs_thr, float(cfg["lit_threshold_fraction"]) * effect_geom_scale)
    lit_bin = white_occ_lum > lit_thr

    lum_log = np.log1p(color_lum / (effect_color_scale + eps)).astype(np.float32)
    lum_log_blur = gaussian_blur(lum_log, float(cfg["hf_blur_sigma"]))
    hf_edge = laplacian_abs(lum_log_blur)
    hf_soft, hf_scale = normalize_by_percentile(hf_edge, 95.0, eps)
    hf_thr = max(eps, float(cfg["hf_threshold_fraction"]) * hf_scale)
    hf_bin = hf_edge > hf_thr

    visibility = np.clip(white_occ_lum / (white_noocc_lum + eps), 0.0, 1.0).astype(np.float32)
    visibility = np.where(possible, visibility, 1.0).astype(np.float32)
    shadow_soft = np.where(possible, 1.0 - visibility, 0.0).astype(np.float32)
    cast_shadow = possible & (visibility < float(cfg["visibility_shadow_threshold"]))
    lit_clear = possible & lit_bin & (visibility > float(cfg["visibility_clear_threshold"]))

    shadow_boundary_source = gaussian_blur(shadow_soft, float(cfg["shadow_boundary_blur_sigma"]))
    shadow_boundary_edge = laplacian_abs(shadow_boundary_source)
    shadow_boundary_soft, shadow_boundary_scale = normalize_by_percentile(shadow_boundary_edge, 95.0, eps)
    shadow_boundary_thr = max(eps, float(cfg["shadow_boundary_threshold_fraction"]) * shadow_boundary_scale)
    shadow_boundary = possible & (shadow_boundary_edge > shadow_boundary_thr)

    object_mask_path = scene_dir / "masks/object_mask.png"
    receiver_mask_path = scene_dir / "masks/receiver_mask.png"
    object_mask_used = False
    object_mask = None
    receiver_mask = None
    receiver_cast_shadow = cast_shadow
    if receiver_mask_path.exists():
        receiver_lum = luminance(load_render_rgb(receiver_mask_path))
        receiver_mask = receiver_lum > 0.5
        receiver_cast_shadow = receiver_cast_shadow & receiver_mask
    if object_mask_path.exists():
        object_lum = luminance(load_render_rgb(object_mask_path))
        object_mask = object_lum > 0.5
        receiver_cast_shadow = receiver_cast_shadow & (~object_mask)

    preserve_union = dilate_binary(
        preserve_effect_color_bin | preserve_effect_geom_bin | shadow_boundary | receiver_cast_shadow | lit_clear,
        int(cfg["preserve_dilate_radius"]),
    )
    preserve = ~preserve_union

    contact_base = receiver_cast_shadow & (visibility < float(cfg["contact_visibility_threshold"]))
    contact_soft = shadow_soft * contact_base.astype(np.float32)
    if object_mask is not None:
        object_near = dilate_binary(object_mask, int(cfg["contact_object_dilate_radius"])) & (~object_mask)
        distance = chamfer_distance_to_mask(object_near)
        sigma = max(float(cfg["contact_soft_sigma"]), eps)
        contact_soft = contact_soft * np.exp(-distance / sigma).astype(np.float32)
        object_mask_used = True
    contact_shadow = contact_soft > float(cfg["contact_soft_threshold"])

    rel_dir = f"spatial/point_lights/light_{light_index:03d}_masks"
    labels: dict[str, str] = {}
    save_soft_label(scene_dir, f"{rel_dir}/effect_soft", effect_color_soft, labels, "effect_soft")
    labels["effect_color_soft"] = labels["effect_soft"]
    labels["effect_color_soft_png"] = labels["effect_soft_png"]
    save_binary_label(scene_dir, f"{rel_dir}/effect_bin.png", effect_color_bin, labels, "effect_bin")
    labels["effect_color_bin"] = labels["effect_bin"]
    save_soft_label(scene_dir, f"{rel_dir}/effect_geom_soft", effect_geom_soft, labels, "effect_geom_soft")
    save_binary_label(scene_dir, f"{rel_dir}/effect_geom_bin.png", effect_geom_bin, labels, "effect_geom_bin")
    save_binary_label(scene_dir, f"{rel_dir}/light_support.png", possible, labels, "light_support")
    save_binary_label(scene_dir, f"{rel_dir}/lit_region.png", lit_bin, labels, "lit_region")
    save_soft_label(scene_dir, f"{rel_dir}/hf_boundary_soft", hf_soft, labels, "hf_boundary_soft")
    labels["hf_contribution_soft"] = labels["hf_boundary_soft"]
    labels["hf_contribution_soft_png"] = labels["hf_boundary_soft_png"]
    save_binary_label(scene_dir, f"{rel_dir}/hf_boundary.png", hf_bin, labels, "hf_boundary")
    labels["hf_contribution"] = labels["hf_boundary"]
    save_binary_label(scene_dir, f"{rel_dir}/preserve.png", preserve, labels, "preserve")
    save_soft_label(scene_dir, f"{rel_dir}/visibility", visibility, labels, "visibility")
    save_soft_label(scene_dir, f"{rel_dir}/shadow_soft", shadow_soft, labels, "shadow_soft")
    save_binary_label(scene_dir, f"{rel_dir}/shadow_possible.png", possible, labels, "shadow_possible")
    save_binary_label(scene_dir, f"{rel_dir}/cast_shadow.png", cast_shadow, labels, "cast_shadow")
    save_binary_label(
        scene_dir,
        f"{rel_dir}/cast_shadow_receiver.png",
        receiver_cast_shadow,
        labels,
        "cast_shadow_receiver",
    )
    save_soft_label(scene_dir, f"{rel_dir}/shadow_boundary_soft", shadow_boundary_soft, labels, "shadow_boundary_soft")
    save_binary_label(scene_dir, f"{rel_dir}/shadow_boundary.png", shadow_boundary, labels, "shadow_boundary")
    save_binary_label(scene_dir, f"{rel_dir}/lit_clear.png", lit_clear, labels, "lit_clear")
    save_soft_label(scene_dir, f"{rel_dir}/contact_shadow_soft", contact_soft, labels, "contact_shadow_soft")
    save_binary_label(scene_dir, f"{rel_dir}/contact_shadow.png", contact_shadow, labels, "contact_shadow")

    stats = {
        "effect_scale_q95": float(effect_color_scale),
        "effect_threshold": float(effect_color_thr),
        "effect_color_scale_q95": float(effect_color_scale),
        "effect_color_threshold": float(effect_color_thr),
        "preserve_effect_color_threshold": float(preserve_effect_color_thr),
        "effect_geom_scale_q95": float(effect_geom_scale),
        "effect_geom_threshold": float(effect_geom_thr),
        "preserve_effect_geom_threshold": float(preserve_effect_geom_thr),
        "lit_threshold": float(lit_thr),
        "hf_scale_q95": float(hf_scale),
        "hf_threshold": float(hf_thr),
        "hf_blur_sigma": float(cfg["hf_blur_sigma"]),
        "white_noocc_scale_q95": float(white_noocc_scale),
        "shadow_possible_threshold": float(possible_thr),
        "visibility_shadow_threshold": float(cfg["visibility_shadow_threshold"]),
        "visibility_clear_threshold": float(cfg["visibility_clear_threshold"]),
        "contact_visibility_threshold": float(cfg["contact_visibility_threshold"]),
        "contact_soft_sigma": float(cfg["contact_soft_sigma"]),
        "contact_soft_threshold": float(cfg["contact_soft_threshold"]),
        "effect_ratio": mask_ratio(effect_color_bin),
        "effect_color_ratio": mask_ratio(effect_color_bin),
        "effect_geom_ratio": mask_ratio(effect_geom_bin),
        "preserve_effect_color_ratio": mask_ratio(preserve_effect_color_bin),
        "preserve_effect_geom_ratio": mask_ratio(preserve_effect_geom_bin),
        "lit_region_ratio": mask_ratio(lit_bin),
        "hf_boundary_ratio": mask_ratio(hf_bin),
        "preserve_ratio": mask_ratio(preserve),
        "shadow_possible_ratio": mask_ratio(possible),
        "cast_shadow_ratio": mask_ratio(cast_shadow),
        "cast_shadow_receiver_ratio": mask_ratio(receiver_cast_shadow),
        "shadow_boundary_ratio": mask_ratio(shadow_boundary),
        "lit_clear_ratio": mask_ratio(lit_clear),
        "contact_shadow_ratio": mask_ratio(contact_shadow),
        "contact_shadow_soft_mean": float(np.mean(contact_soft)),
        "mean_visibility_on_possible": float(np.mean(visibility[possible])) if np.any(possible) else 1.0,
        "receiver_mask_available": bool(receiver_mask is not None),
        "receiver_mask_ratio": mask_ratio(receiver_mask) if receiver_mask is not None else 0.0,
        "object_mask_used_for_contact": bool(object_mask_used),
    }
    stats_rel = f"{rel_dir}/stats.json"
    write_json(scene_dir / stats_rel, stats)
    labels["stats"] = stats_rel
    return {
        "schema": "tokenlight_relighting_labels_v2",
        "valid": True,
        "labels": labels,
...
def render_relighting_auxiliary_and_masks(
    scene_dir: Path,
    rel_base: str,
    light_index: int,
    config: dict,
    ...
    if not bool(config.get("_render_relighting_masks", True)):
        return {}
    white_config = relighting_white_shading_config(config)
    white_occ_output = render_white_diffuse_component(
        scene_dir,
        f"{rel_base}_white_occ",
        config,
        white_config,
        shadows_enabled=True,
    )
    white_noocc_output = render_white_diffuse_component(
        scene_dir,
        f"{rel_base}_white_noocc",
        config,
        white_config,
        shadows_enabled=False,
    )
    label_meta = build_relighting_labels(scene_dir, light_index, color_output, white_occ_output, white_noocc_output, config)
    result = {
        "spotlight_color_occ": component_meta(color_output),
        "spotlight_white_occ": component_meta(white_occ_output),
        "spotlight_white_noocc": component_meta(white_noocc_output),
        "spotlight_white_label_config": {
            "mode": white_config.get("mode"),
            "direct_only": bool(white_config.get("direct_only", False)),
            "preserve_alpha_cutout": bool(white_config.get("preserve_alpha_cutout", False)),
        },
        "relighting_label_schema": label_meta["schema"],
    }
    if label_meta.get("valid"):
        result["labels"] = label_meta["labels"]
        result["label_stats"] = label_meta["stats"]
    else:
        result["label_error"] = label_meta.get("error", "unknown_label_error")
    return result
...
def optical_override_material(
    ...
    preserve_alpha_cutout: bool = False,
) -> bpy.types.Material:
    preserve_source = (preserve_optical and material_is_optical(source)) or (
        preserve_alpha_cutout and material_has_alpha_cutout_name(source)
    )
    if preserve_source:
    ...
    for obj in bpy.data.objects:
        ...
        for source in source_materials:
            obj.data.materials.append(
                optical_override_material(
                    source,
                    material,
                    optical_cache,
                    preserve_optical,
                    preserve_alpha_cutout=preserve_alpha_cutout,
                )
            )
...
def light_shadow_snapshot() -> dict[str, bool]:
    return {
        obj.name: bool(obj.data.use_shadow)
        for obj in bpy.data.objects
        if obj.type == "LIGHT" and hasattr(obj.data, "use_shadow")
    }


def set_all_light_shadows(enabled: bool) -> None:
    for obj in bpy.data.objects:
        if obj.type == "LIGHT" and hasattr(obj.data, "use_shadow"):
            obj.data.use_shadow = bool(enabled)


def restore_light_shadows(snapshot: dict[str, bool]) -> None:
    for obj in bpy.data.objects:
        if obj.type == "LIGHT" and obj.name in snapshot and hasattr(obj.data, "use_shadow"):
            obj.data.use_shadow = bool(snapshot[obj.name])


def render_white_diffuse_component(
    scene_dir: Path,
    rel_base: str,
    config: dict,
    white_config: dict,
    ...
    try:
        ...
        restore_light_shadows(shadow_snapshot)
...
def render_binary_mesh_mask(scene_dir: Path, rel_path: str, include_object) -> str:
    ...
    white = make_emission_mat(f"TL_mask_{safe_name}_white", (1.0, 1.0, 1.0), 1.0)
    black = make_emission_mat(f"TL_mask_{safe_name}_black", (0.0, 0.0, 0.0), 1.0)
    try:
        for obj in bpy.data.objects:
            if obj.type == "MESH":
                obj.data.materials.clear()
                obj.data.materials.append(white if include_object(obj) else black)
...
def render_object_mask(scene_dir: Path, subject_objects: list[bpy.types.Object]) -> str:
    ...
    return render_binary_mesh_mask(scene_dir, "masks/object_mask.png", lambda obj: obj in subject_set)


def receiver_role_for_object(obj: bpy.types.Object) -> str | None:
    if not bool(obj.get("TL_RECEIVER", False)):
        return None
    name = obj.name
    if name.startswith("TL_Ground"):
        return "floor"
    if "Wall" in name:
        return "wall"
    return "receiver"
...
def render_receiver_masks(scene_dir: Path) -> dict[str, str]:
    return {
        "receiver": render_binary_mesh_mask(
            scene_dir,
            "masks/receiver_mask.png",
            lambda obj: receiver_role_for_object(obj) is not None,
        ),
        "floor": render_binary_mesh_mask(
            scene_dir,
            "masks/floor_mask.png",
            lambda obj: receiver_role_for_object(obj) == "floor",
        ),
        "wall": render_binary_mesh_mask(
            scene_dir,
            "masks/wall_mask.png",
            lambda obj: receiver_role_for_object(obj) == "wall",
        ),
    }
...
def render_spatial_components(scene_dir: Path, config: dict, rng: random.Random, camera: bpy.types.Object, center: Vector) -> dict:
        ...
        with progress_bar(range(max_attempts), total=max_attempts, desc=f"{scene_dir.name} point candidates", unit="attempt") as pbar:
            for attempt_index in pbar:
                ...
                if accepted:
                    ...
                    relighting_aux = {}
                    if bool(config.get("_render_relighting_masks", True)):
                        light = create_point_light(
                            f"TL_Point_{light_index:03d}_RelightingAux",
                            p_world,
                            float(light_settings["world_energy"]),
                            world_radius,
                            render_light_color,
                        ...
                        try:
                            relighting_aux = render_relighting_auxiliary_and_masks(
                                scene_dir,
                                rel_base,
                                light_index,
                                config,
                    ...
                    if relighting_aux:
                        light_meta.update(relighting_aux)
                    if diffuse_variants:
    ...
    return {
        ...
        "relighting_masks": {
            "enabled": bool(config.get("_render_relighting_masks", True)),
            "schema": "tokenlight_relighting_labels_v2",
            "config": relighting_mask_config(config),
            "white_occ_semantics": "white_diffuse_direct_light_with_shadow_occlusion",
            "white_noocc_semantics": "white_diffuse_direct_light_with_light_shadows_disabled",
            "effect_soft_semantics": "color_contribution_weight",
            "effect_geom_soft_semantics": "white_occ_lighting_footprint",
            "lit_region_semantics": "conservative_white_occ_lit_region",
        },
...
def render_object_scene(scene_index: int, config: dict, root: Path, only: str) -> dict:
    ...
    mask_paths = {}
    if not white_only:
        mask_paths["object"] = render_object_mask(scene_dir, subject_objects)
        mask_paths.update(render_receiver_masks(scene_dir))
...
def main() -> int:
    ...
    config["_render_relighting_masks"] = bool(
        args.relighting_masks and not config["_debug_preview_only"] and not config["_pbr_white_shading_only"]
    )
    ...
    receiver_texture_status = configure_receiver_textures(
        config,
        root,
    ...
    manifest = {
        ...
        "settings": {
            "object_target_size": float(config["object"].get("target_size", 1.2)),
            "object_min_size": float(config["object"].get("min_size", config["object"].get("target_size", 1.2))),
            "rig_reference_object_size": float(config["canonical"]["reference_object_size"]),
            "canonical_world_scale": float(config["canonical"]["world_scale"]),
            "canonical_scale_multiplier_range": config["canonical"]["scale_multiplier_range"],
