...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Objaverse-XL objects and write manifests/objects.txt for this TokenLight dataset."
    )
    ...
    parser.add_argument(
        "--numbered-out-dir",
        default="assets/objects/objaverse_xl",
...
def materialize(src: Path, dst: Path, mode: str, overwrite: bool, fallback_copy: bool) -> bool:
    if not src.exists():
        print(f"[WARN] Missing downloaded file, skip: {src}")
        return False

    dst.parent.mkdir(parents=True, exist_ok=True)
    if dst.exists() or dst.is_symlink():
        if not overwrite:
            print(f"[WARN] Existing output, skip: {dst}")
            return False
        dst.unlink()
    ...
    try:
        ...
        if not fallback_copy:
            raise
        print(f"[WARN] {mode} failed for {dst}: {exc}. Falling back to copy.")
        shutil.copy2(src, dst)
        return True
...
def write_object_manifest(lines: Iterable[str], path: Path, append: bool) -> None:
    ...
    with path.open("w", encoding="utf-8") as f:
        for line in merged:
            f.write(line + "\n")
    print(f"Wrote renderer object manifest: {path} ({len(merged)} entries)")
...
def main() -> int:
    ...
    print(f"[1] Loading Objaverse-XL annotations into: {download_dir}")
    annotations = oxl.get_annotations(download_dir=str(download_dir), refresh=args.refresh)
    print(f"[2] Total annotations: {len(annotations):,}")
    print(f"[2] Columns: {list(annotations.columns)}")
    ...
    print(f"\n[3] Filtered objects: {len(filtered):,}")
    if len(filtered) == 0:
    ...
    print(f"[4] Selected {len(selected):,} objects. Metadata: {selection_csv}")
    ...
    print(f"[6] Downloaded/found objects: {len(downloaded):,}")
    ...
    print(f"[7] Download manifest: {manifest_csv}")
    ...
    if args.numbered_mode == "none":
        ...
        print(f"[8] Numbered objects: {len(numbered_records):,} -> {numbered_dir}")
