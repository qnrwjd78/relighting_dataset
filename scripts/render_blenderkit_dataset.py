"""Download and render the classified BlenderKit scene dataset."""
...
DEFAULT_CONFIG = "configs/tokenlight_synthetic_full_ratio3p5_cube1p6.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--manual-decisions-dir", default="data/blenderkit/manual_scene_decisions")
    parser.add_argument("--manual-object-modes", nargs="+", default=["object_group_manual"])
    parser.add_argument("--manual-object-confidence", nargs="+", default=["high", "medium"])
    parser.add_argument("--use-manual-object-decisions", dest="use_manual_object_decisions", action="store_true", default=True)
    parser.add_argument("--no-manual-object-decisions", dest="use_manual_object_decisions", action="store_false")
    ...
    parser.add_argument("--random-light-grid", action="store_false", dest="fixed_light_grid", default=False)
    parser.add_argument("--point-light-valid-filter", dest="point_light_valid_filter", action="store_true", default=True)
    parser.add_argument("--no-point-light-valid-filter", dest="point_light_valid_filter", action="store_false", default=True)
    parser.add_argument(
        "--light-volume-placement",
        choices=["bbox-center", "camera-framed", "camera-center-object-depth-scale"],
    ...
    parser.add_argument(
        "--camera-mode",
        choices=["source-preserve", "source-dolly", "source-lookat-projection", "source-lookat-dolly", "source", "objaverse"],
        default="source-lookat-dolly",
        help=(
            "source-preserve keeps the .blend camera unchanged and rescales the canonical light volume to match "
            "the canonical depth; source-dolly keeps the source view axes/FOV policy and moves only along the "
            "source view axis; source-lookat-projection keeps source camera position, looks at the selected "
            "object center, applies the canonical FOV, and scales the light cube to match the Objaverse camera "
            "depth ratio; source-lookat-dolly keeps the source viewing direction to the object, applies the "
            "canonical FOV, and places the camera at canonical depth from the selected object; source keeps both "
            "camera and bbox-derived scale unchanged; objaverse creates a new TokenLight camera."
        ),
    )
    parser.add_argument(
        "--spatial-bbox-mode",
...
def load_manual_object_decisions(args: argparse.Namespace) -> dict[str, dict]:
    if not args.use_manual_object_decisions:
        return {}
    path = resolve_repo_path(args.manual_decisions_dir) / "single_scene_light_good_object_decisions.json"
    if not path.exists():
        print(f"[BlenderKitSpatial] Manual object decisions missing: {path}", file=sys.stderr)
        return {}

    allowed_modes = set(args.manual_object_modes)
    allowed_confidence = set(args.manual_object_confidence)
    ...
    for row in load_json(path):
        ...
        if row.get("confidence") not in allowed_confidence:
            continue
        if not row.get("candidate_objects"):
            continue
        decisions[scene_id] = row
    return decisions
...
def selected_ids_from_url_list(classification: Path, by_id: dict[str, dict]) -> tuple[list[str], list[str]]:
    uuid_to_id: dict[str, str] = {}
    for item_id, item in by_id.items():
        for key in ("asset_id", "asset_base_id"):
            value = str(item.get(key, "")).strip().lower()
            if value:
                uuid_to_id[value] = item_id
    ...
    for raw in classification.read_text(encoding="utf-8").splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        direct = re.search(r"blenderkit_(\d+)", line)
        if direct:
            item_id = direct.group(1).zfill(5)
        else:
            match = re.search(r"asset-gallery-detail/([0-9a-fA-F-]+)", line)
            if not match:
                continue
            uuid_value = match.group(1).lower()
            item_id = uuid_to_id.get(uuid_value)
            if item_id is None:
                unmatched.append(uuid_value)
                continue
...
def load_selected_items(args: argparse.Namespace) -> list[dict]:
    classification = resolve_repo_path(args.classification)
    index = load_json(resolve_repo_path(args.index_json))
    by_id = {str(item.get("id")).zfill(5): item for item in index.get("items", [])}
    if classification.exists():
        ids = selected_ids(classification, set(args.categories))
        if not ids:
            ids, unmatched = selected_ids_from_url_list(classification, by_id)
            if ids:
                print(f"[BlenderKitSpatial] URL/direct-id list matched {len(ids)} item(s) from {classification}")
            for uuid_value in unmatched:
                print(f"[BlenderKitSpatial] WARNING URL UUID not found in index-json: {uuid_value}", file=sys.stderr)
    else:
        ids = sorted(by_id)
        print(
            f"[BlenderKitSpatial] Classification missing; using all {len(ids)} indexed scene(s): {classification}"
        )
    decisions = load_manual_object_decisions(args)
    items = []
    for item_id in ids:
        ...
        if scene_id in decisions:
            item["manual_object_decision"] = decisions[scene_id]
        items.append(item)
...
def worker_command(args: argparse.Namespace, blend_path: Path, item_path: Path) -> list[str]:
    cmd = shlex.split(args.blender_cmd) + [
        "-b",
        "--python",
        str(ROOT / "scripts" / "render_blenderkit_dataset.py"),
        "--",
        "--worker",
        "--blend",
        str(blend_path),
    ...
    cmd.extend(["--camera-mode", args.camera_mode])
    ...
    if args.point_light_valid_filter is True:
        cmd.append("--point-light-valid-filter")
    elif args.point_light_valid_filter is False:
        cmd.append("--no-point-light-valid-filter")
    if args.pbr:
        cmd.append("--pbr")
    return cmd
...
def orchestrate(args: argparse.Namespace) -> int:
    ...
    manual_count = sum(1 for item in items if item.get("manual_object_decision"))
    print(f"[BlenderKitSpatial] Selected items: {len(items)}")
    print(f"[BlenderKitSpatial] Manual object decisions attached: {manual_count}")
...
def load_runtime_config(config_path: Path, args: argparse.Namespace) -> dict:
    ...
    if args.point_light_valid_filter is not None:
        config.setdefault("spatial", {}).setdefault("valid_filter", {})["enabled"] = bool(args.point_light_valid_filter)
    ...
    config["_runtime"] = {
        ...
        "receiver_materials": [],
        "target_depth_over_scale": args.light_volume_depth_over_scale,
    }
    if args.light_volume_depth_over_scale is not None:
        base_depth = base_canonical_depth_over_scale(config)
        target_depth = max(abs(float(args.light_volume_depth_over_scale)), 1e-6)
        ratio = target_depth / max(base_depth, 1e-6)
        axis_scale = config.setdefault("canonical", {}).get("axis_scale", [1.0, 1.0, 1.0])
        if isinstance(axis_scale, dict):
            adjusted = {
                "x": float(axis_scale.get("x", 1.0)) * ratio,
                "y": float(axis_scale.get("y", 1.0)) * ratio,
                "z": float(axis_scale.get("z", 1.0)) * ratio,
            }
        elif isinstance(axis_scale, (int, float)):
            adjusted = [float(axis_scale) * ratio] * 3
        else:
            adjusted = [
                (float(axis_scale[i]) if i < len(axis_scale) else 1.0) * ratio
                for i in range(3)
            ]
        config["canonical"]["axis_scale"] = adjusted
        config["_runtime"]["axis_scale_adjustment"] = {
            "mode": "projection_preserve_depth_ratio_override",
            "base_depth_over_scale": base_depth,
            "target_depth_over_scale": target_depth,
            "scale_factor": ratio,
            "axis_scale": adjusted,
        }
    return config
...
def select_manual_object_bounds(item: dict, bpy_module, relight) -> tuple | None:
    decision = item.get("manual_object_decision")
    if not decision:
        return None

    selected = []
    missing = []
    for name in decision.get("candidate_objects") or []:
        obj = bpy_module.data.objects.get(str(name))
    ...
    bbox_min, bbox_max = relight.mesh_bbox(selected)
    source = f"manual_object_decision:{decision.get('mode')}:{decision.get('confidence')}"
    meta = {
        "scene_id": decision.get("scene_id"),
        "mode": decision.get("mode"),
        "confidence": decision.get("confidence"),
        "render_use_recommendation": decision.get("render_use_recommendation"),
        "candidate_objects": [obj.name for obj in selected],
        "missing_candidate_objects": missing,
        "candidate_bbox_world": decision.get("candidate_bbox_world"),
    }
    return bbox_min, bbox_max, source, meta


def base_canonical_depth_over_scale(config: dict) -> float:
...
def canonical_depth_over_scale(config: dict) -> float:
    runtime_target = config.get("_runtime", {}).get("target_depth_over_scale")
    if runtime_target is not None:
        return max(abs(float(runtime_target)), 1e-6)
    return base_canonical_depth_over_scale(config)
...
def apply_canonical_fov(config: dict, camera) -> dict:
    original = camera_fov_snapshot(camera)
    target_degrees = float(config.get("camera", {}).get("fov_degrees", 39.6))
    data = camera.data
    data.type = "PERSP"
    data.angle = math.radians(target_degrees)
    return {
        "mode": "canonical_fov",
        "target_fov_degrees": target_degrees,
        "original": original,
        "applied": camera_fov_snapshot(camera),
    }


def freeze_source_camera(camera) -> dict:
    matrix = camera.matrix_world.copy()
    parent_name = camera.parent.name if camera.parent is not None else None
    constraints = [constraint.name for constraint in camera.constraints]
    had_object_animation = camera.animation_data is not None
    had_data_animation = camera.data.animation_data is not None
    ...
    return {
        "object_animation_cleared": had_object_animation,
        "data_animation_cleared": had_data_animation,
        "parent_cleared": parent_name,
        "constraints_removed": constraints,
    }


def source_camera_dolly_to_canonical_depth(config: dict, camera, bbox_center, relight, Vector) -> dict:
    right, up, forward = relight.camera_basis(camera)
    original_location = Vector(camera.location).copy()
    original_depth = float((bbox_center - original_location).dot(forward))
    target_depth_over_scale = canonical_depth_over_scale(config)
    target_depth = target_depth_over_scale * relight.canonical_world_scale(config)
    camera.location = original_location + forward * (original_depth - target_depth)
    relight.bpy.context.scene.camera = camera
    relight.bpy.context.view_layer.update()
    new_depth = float((bbox_center - Vector(camera.location)).dot(forward))
    return {
        "mode": "source-camera-dolly-only",
        "preserved": {
            "rotation": True,
            "fov": False,
            "sensor_shift": True,
            "view_plane_position": True,
        },
        "original_location": relight.vec_to_list(original_location),
        "location": relight.vec_to_list(Vector(camera.location)),
        "camera_delta": relight.vec_to_list(Vector(camera.location) - original_location),
        "dolly_axis_world": relight.vec_to_list(forward),
        "bbox_center": relight.vec_to_list(bbox_center),
        "original_bbox_camera_depth": original_depth,
        "bbox_camera_depth": new_depth,
        "target_depth": target_depth,
        "target_depth_over_scale": target_depth_over_scale,
        "canonical_scale": relight.canonical_world_scale(config),
        "canonical_axis_scale": relight.vec_to_list(relight.canonical_axis_scale(config)),
        "camera_distance_over_scale": float(new_depth / max(relight.canonical_world_scale(config), 1e-6)),
        "source_camera_freeze": config.get("_runtime", {}).get("source_camera_freeze"),
        "fov_adjustment": config.get("_runtime", {}).get("source_camera_fov_adjustment"),
        "source_world_axes": {
            "x_axis_world": relight.vec_to_list(right),
            "y_axis_world": relight.vec_to_list(forward),
            "z_axis_world": relight.vec_to_list(up),
        },
    }


def source_camera_lookat_projection_match(config: dict, camera, bbox_center, bbox_min, bbox_max, relight, Vector) -> dict:
    original_location = Vector(camera.location).copy()
    original_rotation = [float(v) for v in camera.rotation_euler]
    original_fov = camera_fov_snapshot(camera)
    fov_adjustment = apply_canonical_fov(config, camera)
    relight.look_at(camera, bbox_center)
    relight.bpy.context.scene.camera = camera
    relight.bpy.context.view_layer.update()

    right, up, forward = relight.camera_basis(camera)
    depth = max(float((bbox_center - original_location).dot(forward)), 1e-6)
    target_depth_over_scale = canonical_depth_over_scale(config)
    ...
    projection_scale = max(depth / target_depth_over_scale, 1e-6)
    bbox_scale = relight.compute_canonical_scale(config, bbox_min, bbox_max)
    config.setdefault("_runtime", {})["canonical_scale"] = projection_scale
    return {
        "mode": "source-camera-lookat-projection-match",
        "preserved": {
            "location": True,
            "rotation": False,
            "fov": True,
        ...
        "location": relight.vec_to_list(Vector(camera.location)),
        "camera_delta": relight.vec_to_list(Vector(camera.location) - original_location),
        "original_rotation_euler": original_rotation,
        "rotation_euler": [float(v) for v in camera.rotation_euler],
        "bbox_center": relight.vec_to_list(bbox_center),
        "bbox_camera_depth": depth,
        "target_depth_over_scale": target_depth_over_scale,
        ...
        "fov_scale_axis": "x",
        "bbox_derived_canonical_scale": bbox_scale,
        "canonical_scale": relight.canonical_world_scale(config),
        "canonical_axis_scale": relight.vec_to_list(relight.canonical_axis_scale(config)),
        "camera_distance_over_scale": float(depth / max(relight.canonical_world_scale(config), 1e-6)),
        "projection_match": {
            "formula": "scale = depth / target_depth_over_scale after applying canonical FOV",
            "matches_objaverse_camera_depth_ratio": True,
            "matches_objaverse_fov": True,
        },
        "source_camera_freeze": config.get("_runtime", {}).get("source_camera_freeze"),
        "fov": {
            "original": original_fov,
            "adjustment": fov_adjustment,
            "applied": camera_fov_snapshot(camera),
        },
        "source_world_axes": {
            "x_axis_world": relight.vec_to_list(right),
...
def source_camera_lookat_dolly_to_object_scale(config: dict, camera, bbox_center, relight, Vector) -> dict:
    original_location = Vector(camera.location).copy()
    original_rotation = [float(v) for v in camera.rotation_euler]
    original_fov = camera_fov_snapshot(camera)
    fov_adjustment = apply_canonical_fov(config, camera)

    view_direction = bbox_center - original_location
    if float(view_direction.length) < 1e-6:
    ...
    target_depth_over_scale = canonical_depth_over_scale(config)
    canonical_scale = relight.canonical_world_scale(config)
    target_depth = target_depth_over_scale * canonical_scale
    camera.location = bbox_center - view_direction * target_depth
    relight.look_at(camera, bbox_center)
    relight.bpy.context.scene.camera = camera
    relight.bpy.context.view_layer.update()

    right, up, forward = relight.camera_basis(camera)
    depth = max(float((bbox_center - Vector(camera.location)).dot(forward)), 1e-6)
    return {
        "mode": "source-camera-lookat-dolly-object-scale",
        "preserved": {
            "location": False,
            "rotation": False,
            "fov": False,
            "sensor_shift": True,
            "view_plane_position": False,
        },
        "original_location": relight.vec_to_list(original_location),
        "location": relight.vec_to_list(Vector(camera.location)),
        "camera_delta": relight.vec_to_list(Vector(camera.location) - original_location),
        "source_to_object_direction_world": relight.vec_to_list(view_direction),
        "original_rotation_euler": original_rotation,
        "rotation_euler": [float(v) for v in camera.rotation_euler],
        "bbox_center": relight.vec_to_list(bbox_center),
        "bbox_camera_depth": depth,
        ...
        "canonical_axis_scale": relight.vec_to_list(relight.canonical_axis_scale(config)),
        "camera_distance_over_scale": float(depth / max(canonical_scale, 1e-6)),
        "source_camera_freeze": config.get("_runtime", {}).get("source_camera_freeze"),
        "fov": {
            "original": original_fov,
            "adjustment": fov_adjustment,
...
def source_camera_preserve_match_canonical_depth(config: dict, camera, bbox_center, bbox_min, bbox_max, relight, Vector) -> dict:
    right, up, forward = relight.camera_basis(camera)
    location = Vector(camera.location).copy()
    depth = max(float((bbox_center - location).dot(forward)), 1e-6)
    target_depth_over_scale = canonical_depth_over_scale(config)
    bbox_scale = relight.compute_canonical_scale(config, bbox_min, bbox_max)
    matched_scale = depth / target_depth_over_scale
    config.setdefault("_runtime", {})["canonical_scale"] = max(matched_scale, 1e-6)
    return {
        "mode": "source-camera-preserve-scale-match",
        "preserved": {
            "location": True,
            "rotation": True,
            "fov": True,
        ...
        "location": relight.vec_to_list(location),
        "bbox_center": relight.vec_to_list(bbox_center),
        "bbox_camera_depth": depth,
        "target_depth_over_scale": target_depth_over_scale,
        "canonical_scale": relight.canonical_world_scale(config),
        "bbox_derived_canonical_scale": bbox_scale,
        "scale_rule": "source_camera_depth_over_canonical_distance",
        "canonical_axis_scale": relight.vec_to_list(relight.canonical_axis_scale(config)),
        "camera_distance_over_scale": float(depth / max(relight.canonical_world_scale(config), 1e-6)),
        "source_world_axes": {
            "x_axis_world": relight.vec_to_list(right),
...
def camera_center_object_depth_scale_light_volume(
    config: dict,
    camera,
    bbox_center,
    relight,
    ...
    _right, _up, forward = relight.camera_basis(camera)
    camera_location = Vector(camera.location)
    object_depth = float((bbox_center - camera_location).dot(forward))
    if object_depth <= 1e-6:
        object_depth = float((bbox_center - camera_location).length)
    target_ratio = float(target_depth_over_scale) if target_depth_over_scale is not None else canonical_depth_over_scale(config)
    target_ratio = max(target_ratio, 1e-6)
    light_center = camera_location + forward * object_depth
    scale = max(object_depth / target_ratio, 1e-6)
    config.setdefault("_runtime", {})["canonical_scale"] = scale
    adjustment = {
        "mode": "camera-center-object-depth-scale",
        "bbox_center": relight.vec_to_list(bbox_center),
        "adjusted_center": relight.vec_to_list(light_center),
        "center_shift": relight.vec_to_list(light_center - bbox_center),
        "camera_location": relight.vec_to_list(camera_location),
        "object_camera_depth": object_depth,
        "cube_camera_depth": float((light_center - camera_location).dot(forward)),
        "target_depth_over_scale": target_ratio,
        "canonical_camera_distance": target_ratio,
        "scale": scale,
        "scale_rule": "object_depth_over_canonical_camera_distance",
        "axis_scale": relight.vec_to_list(relight.canonical_axis_scale(config)),
    }
    config["_runtime"]["light_volume_center_source"] = "camera_optical_axis_at_selected_object_depth"
    config["_runtime"]["light_volume_adjustment"] = adjustment
    return light_center, adjustment
...
def worker(args: argparse.Namespace) -> int:
    ...
    manual_bounds = select_manual_object_bounds(item, bpy, relight)
    manual_object_meta = None
    if manual_bounds is not None:
        bbox_min, bbox_max, bbox_source, manual_object_meta = manual_bounds
    else:
    ...
    if item.get("camera") and item["camera"] in bpy.data.objects:
        source_camera = bpy.data.objects[item["camera"]]
    elif bpy.context.scene.camera:
        source_camera = bpy.context.scene.camera

    camera_meta = None
    camera_adjustment = None
    source_camera_original = (
        {
            "name": source_camera.name,
            "location": relight.vec_to_list(Vector(source_camera.location)),
            "fov": camera_fov_snapshot(source_camera),
        }
        if source_camera is not None
        else None
    )
    if args.camera_mode == "objaverse":
        rng_for_camera = random.Random(int(config["seed"]) + int(item_id))
        camera, camera_meta = relight.create_camera(config, rng_for_camera, bbox_center)
        camera.name = f"TL_ObjaverseCamera_{scene_id}"
        camera["tl_camera_mode"] = "objaverse_canonical_rig"
    elif args.camera_mode == "source-preserve":
        if source_camera is not None:
            camera = source_camera
            bpy.context.scene.camera = camera
            camera_adjustment = source_camera_preserve_match_canonical_depth(
                config,
                camera,
                bbox_center,
                bbox_min,
            ...
            camera["tl_camera_mode"] = "source_preserve_scale_match"
        else:
            rng_for_camera = random.Random(int(config["seed"]) + int(item_id))
            camera, camera_meta = relight.create_camera(config, rng_for_camera, bbox_center)
            camera.name = f"TL_ObjaverseCameraFallback_{scene_id}"
    elif args.camera_mode == "source-dolly":
        if source_camera is not None:
            rng_for_camera = random.Random(int(config["seed"]) + int(item_id))
            relight.apply_scale_multiplier(config, rng_for_camera)
            camera = source_camera
            config.setdefault("_runtime", {})["source_camera_freeze"] = freeze_source_camera(camera)
            config.setdefault("_runtime", {})["source_camera_fov_adjustment"] = apply_canonical_fov(config, camera)
            camera_adjustment = source_camera_dolly_to_canonical_depth(config, camera, bbox_center, relight, Vector)
            camera["tl_camera_mode"] = "source_dolly_canonical_depth"
        else:
            rng_for_camera = random.Random(int(config["seed"]) + int(item_id))
            camera, camera_meta = relight.create_camera(config, rng_for_camera, bbox_center)
            camera.name = f"TL_ObjaverseCameraFallback_{scene_id}"
    elif args.camera_mode == "source-lookat-projection":
        if source_camera is not None:
            camera = source_camera
            config.setdefault("_runtime", {})["source_camera_freeze"] = freeze_source_camera(camera)
            camera_adjustment = source_camera_lookat_projection_match(
                config,
                camera,
                bbox_center,
                bbox_min,
            ...
            camera["tl_camera_mode"] = "source_lookat_projection_match"
        else:
            rng_for_camera = random.Random(int(config["seed"]) + int(item_id))
            camera, camera_meta = relight.create_camera(config, rng_for_camera, bbox_center)
            camera.name = f"TL_ObjaverseCameraFallback_{scene_id}"
    elif args.camera_mode == "source-lookat-dolly":
        if source_camera is not None:
            rng_for_camera = random.Random(int(config["seed"]) + int(item_id))
            relight.apply_scale_multiplier(config, rng_for_camera)
            camera = source_camera
            config.setdefault("_runtime", {})["source_camera_freeze"] = freeze_source_camera(camera)
            camera_adjustment = source_camera_lookat_dolly_to_object_scale(config, camera, bbox_center, relight, Vector)
            camera["tl_camera_mode"] = "source_lookat_dolly_object_scale"
        else:
            rng_for_camera = random.Random(int(config["seed"]) + int(item_id))
            camera, camera_meta = relight.create_camera(config, rng_for_camera, bbox_center)
            camera.name = f"TL_ObjaverseCameraFallback_{scene_id}"
    ...
    light_center = bbox_center
    if args.camera_mode == "objaverse":
        similarity = config.get("_runtime", {}).get("similarity_transform") or {}
        if similarity.get("target_center"):
            light_center = Vector(similarity["target_center"])
        config.setdefault("_runtime", {})["light_volume_center_source"] = "objaverse_canonical_camera_target"
        config["_runtime"]["light_volume_adjustment"] = {
            "mode": "objaverse-canonical-rig",
            "camera_mode": args.camera_mode,
            "bbox_center": relight.vec_to_list(bbox_center),
            "adjusted_center": relight.vec_to_list(light_center),
            "center_shift": relight.vec_to_list(light_center - bbox_center),
            "scale": relight.canonical_world_scale(config),
            "canonical_camera_distance": camera_meta.get("canonical_distance") if camera_meta else None,
            "axis_scale": relight.vec_to_list(relight.canonical_axis_scale(config)),
        }
    elif args.camera_mode == "source-preserve":
        config.setdefault("_runtime", {})["light_volume_center_source"] = "manual_or_candidate_bbox_center_source_camera_preserve"
        config["_runtime"]["light_volume_adjustment"] = {
            "mode": "source-camera-preserve-scale-match",
            "camera_mode": args.camera_mode,
            "bbox_center": relight.vec_to_list(bbox_center),
            ...
            "center_shift": [0.0, 0.0, 0.0],
            "scale": relight.canonical_world_scale(config),
            "canonical_camera_distance": canonical_depth_over_scale(config),
            "axis_scale": relight.vec_to_list(relight.canonical_axis_scale(config)),
            "camera_adjustment": camera_adjustment,
        }
    elif args.camera_mode == "source-dolly":
        config.setdefault("_runtime", {})["light_volume_center_source"] = "manual_or_candidate_bbox_center_source_camera_dolly"
        config["_runtime"]["light_volume_adjustment"] = {
            "mode": "source-camera-dolly-only",
            "camera_mode": args.camera_mode,
            "bbox_center": relight.vec_to_list(bbox_center),
        ...
        config.setdefault("_runtime", {})["light_volume_center_source"] = "manual_or_candidate_bbox_center_source_camera_lookat_projection"
        config["_runtime"]["light_volume_adjustment"] = {
            "mode": "source-camera-lookat-projection-match",
            "camera_mode": args.camera_mode,
            "bbox_center": relight.vec_to_list(bbox_center),
        ...
        config.setdefault("_runtime", {})["light_volume_center_source"] = "manual_or_candidate_bbox_center_source_camera_lookat_dolly"
        config["_runtime"]["light_volume_adjustment"] = {
            "mode": "source-camera-lookat-dolly-object-scale",
            "camera_mode": args.camera_mode,
            "bbox_center": relight.vec_to_list(bbox_center),
    ...
    elif args.light_volume_placement == "camera-center-object-depth-scale":
        light_center, _light_volume_adjustment = camera_center_object_depth_scale_light_volume(
            config,
            camera,
            bbox_center,
            relight,
    ...
    elif args.light_volume_placement == "camera-framed":
    ...
    meta = {
        ...
        "object": {
            ...
            "manual_object_decision": manual_object_meta,
        ...
        "camera": {
            ...
            "source_camera_original": source_camera_original,
        },
