...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Objaverse-XL objects and write asset metadata/manifests."
    )
    parser.add_argument("--download-dir", default="data/objaverse_xl", help="Objaverse-XL metadata/object cache dir.")
    parser.add_argument("--report-out", default="metadata/objaverse_xl/reports", help="Where source/fileType reports are saved.")
    ...
    parser.add_argument("--start", "--start-index", dest="start", type=int, default=0, help="Start offset within the filtered annotation table.")
    parser.add_argument(
        "--selection-manifest",
        default=None,
        help=(
            "Download the exact object ids, in order, from an existing renderer manifest. "
            "When set, --start and --limit are ignored."
        ),
    )
    ...
    parser.add_argument(
        "--write-manifest",
        default="metadata/objaverse_xl/objaverse_xl_objects.txt",
...
def object_identifier_key(value: object) -> str:
    text = str(value).strip().rstrip("/")
    return Path(text).stem if "/" not in text else Path(text.split("?")[0]).stem


def read_selection_keys(path: Path) -> list[str]:
    if not path.is_file():
        raise SystemExit(f"Selection manifest does not exist: {path}")
    keys = [
        object_identifier_key(line)
        for line in path.read_text(encoding="utf-8").splitlines()
        if line.strip() and not line.lstrip().startswith("#")
    ]
    if not keys:
        raise SystemExit(f"Selection manifest is empty: {path}")
    if len(set(keys)) != len(keys):
        raise SystemExit(f"Selection manifest contains duplicate object ids: {path}")
    return keys
...
def main() -> int:
    ...
    if not args.selection_manifest and args.limit <= 0:
    ...
    if args.selection_manifest:
        selection_path = resolve_repo_path(args.selection_manifest)
        selection_keys = read_selection_keys(selection_path)
        filtered = filtered.copy()
        filtered["identifier_key"] = filtered["fileIdentifier"].map(object_identifier_key)
        rows_by_key = {str(row["identifier_key"]): row for _, row in filtered.iterrows()}
        missing = [key for key in selection_keys if key not in rows_by_key]
        if missing:
            preview = ", ".join(missing[:10])
            raise SystemExit(
                f"Selection manifest contains {len(missing)} ids missing from the filtered annotations: {preview}"
            )
        selected = pd.DataFrame([rows_by_key[key] for key in selection_keys]).reset_index(drop=True)
        progress_write(f"[3] Exact selection manifest: {selection_path} ({len(selected):,} objects)")
    else:
    ...
    downloaded_by_key = {object_identifier_key(identifier): path for identifier, path in downloaded.items()}
    manifest_records = []
    for _, row in selected.iterrows():
        file_identifier = str(row["fileIdentifier"])
        local_path = downloaded_by_key.get(object_identifier_key(file_identifier))
        if local_path is None:
            progress_write(f"[WARN] Download result missing for selected object: {file_identifier}")
            continue
