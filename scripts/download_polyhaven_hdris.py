...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download 2k Poly Haven HDRIs by category and write manifests/hdris.txt."
    )
    ...
    parser.add_argument("--out-dir", default="assets/polyhaven/hdris")
    parser.add_argument("--manifest", default="manifests/hdris.txt")
    parser.add_argument("--metadata-out", default="manifests/hdris_polyhaven_metadata.json")
...
def download_file(url: str, path: Path, expected_md5: str | None, overwrite: bool, user_agent: str) -> None:
    if path.exists() and not overwrite:
        if expected_md5 and md5sum(path) != expected_md5:
            print(f"[PolyHaven] Existing file has wrong md5, re-downloading: {path}")
        else:
            print(f"[PolyHaven] Exists: {path}")
            return
    ...
    print(f"[PolyHaven] Downloaded: {path}")
...
def main() -> int:
    ...
    for category in args.categories:
        ...
        for asset_id, meta in order_assets(assets, args.sort, rng):
            ...
            if len(chosen) >= args.per_category:
                break
        print(f"[PolyHaven] {category}: selected {len(chosen)} / {len(assets)} HDRIs")
    ...
    for i, item in enumerate(selected, 1):
        ...
        try:
            ...
            print(f"[PolyHaven] Skipping {asset_id}: missing {args.resolution} {args.format}", file=sys.stderr)
            continue
        ...
        print(f"[PolyHaven] {i:03d}/{len(selected):03d} {item['category']} {asset_id}")
        if not args.dry_run:
            try:
    ...
    if not args.dry_run:
        ...
        print(f"[PolyHaven] Wrote metadata: {metadata_path}")
    else:
        print("[PolyHaven] Dry run; no files or manifest were downloaded/written.")
    return 0
